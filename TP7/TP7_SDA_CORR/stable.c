...
void insertion_sort(int* array, int size)
{
	...
	for (i=1;i<size;i++)
	{
		...
		j=i-1;
		while ((j>=0) && (array[j]/10>current/10))
		{
...
int partition(int* array, int deb, int fin, int pivot)
{
	...
	for (i=deb;i<fin;i++)
	{
		if (array[i]/10 <= val_pivot/10) // place les elts<pivots
		{
...
void fusion2parts_nonstable(int* array, int deb, int mil, int fin)
{
...
void fusion2parts(int* array, int deb, int mil, int fin)
{
	...
	for (k = deb; k <= fin; k++)
	{
		if (i <= mil && (j > fin || array[i]/10 <= array[j]/10))
		{
			tempo[k] = array[i];
			i = i + 1;
		}
...
void fusion_sort(int* array, int deb, int fin)
{
	...
	{
		if (array[fin]/10 < array[deb]/10)	// tri facile
			swap_elt(array,deb,fin);
	}
}
